...
$('body').append('<style>body{background:#000000b3!important;overflow:hidden}#peringatan span{font-size:50px}#peringatan{z-index:999999999;position:fixed;top:0;right:0;left:0;padding:20% 0;height:100%;text-align:center;background:#000000b3;color:#fff}@media only screen and (max-width: 680px){#peringatan{padding:60% 0;}</style><div id="peringatan"><h4 style="margin-bottom: 35px;font-size:x-large;">🔒️ Template is Locked Up</h4><span id="aktivasi">'+second+'</span><p style="margin-top: 20px;font-size: 18px;letter-spacing: 2px;line-height: 30px;">Untuk mengaktifkan template, silahkan klik <a style="color: #eeff00;" href="https://daffadev.my.id/" target="_blank">DISINI</a></p></div>');
setInterval(function () {
second <= 1 ? window.location.href = "$('#url-blog').text()" : $('#aktivasi').text(--second);
}, 1e3)
}
});
...
fixedMenu = 'undefined' == typeof fixedMenu || fixedMenu, $('#blogger-unang-main-nav').menublog(), $('#blogger-unang-main-nav .widget').addClass('show-menu'), $('.show-search').on('click', function () {
    ...
    $('.entry-image-wrap .entry-thumb,.author-avatar-wrap .author-avatar').lazyify(), $('#blogger-unang-mobile-menu').each(function () {
        ...
        $('a#unang').length || (window.location.href = 'https://www.daffadev.my.id/'), $('a#unang:visible').length || (window.location.href = 'https://daffadev.my.id/')
